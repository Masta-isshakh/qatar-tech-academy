...
export function StatTile({ value, label }: { value: string; label: string }) {
  return (
    <div className="flex flex-col gap-1 rounded-2xl border border-border-subtle bg-background p-6 text-center">
      <span className="text-3xl font-extrabold text-primary md:text-4xl">{value}</span>
      ...
      {steps.map((step, i) => (
        <FadeUp as="li" key={step.title} delay={i * 0.05}>
          <div className="flex h-full flex-col gap-2 rounded-2xl border border-border-subtle bg-background p-6">
            <span
              className="ltr-nums grid size-9 place-items-center rounded-full bg-maroon-soft text-sm font-extrabold text-primary"
              aria-hidden
            >
              {i + 1}
            </span>
...
export function TestimonialCard({
  ...
  return (
    <figure className="flex h-full flex-col gap-4 rounded-2xl border border-border-subtle bg-background p-7">
        ...
        {[0, 1].map((copy) => (
          <ul key={copy} className="flex gap-3" aria-hidden={copy === 1}>
            {items.map((name) => (
              <li
                key={`${copy}-${name}`}
                className="whitespace-nowrap rounded-full border border-border-subtle bg-background px-5 py-2.5 text-sm font-semibold text-muted"
              >
                {name}
              </li>
            ))}
...
export function CertificationBadge({
  ...
  return (
    <div className="flex flex-col gap-2 rounded-2xl border border-border-subtle bg-background p-5">
...
export function TrainerCard({
  ...
  return (
    <div className="flex h-full flex-col gap-3 rounded-2xl border border-border-subtle bg-background p-6">
      <div className="relative size-20 overflow-hidden rounded-full">
        <MediaImage src={photoKey || '/images/team/placeholder.jpg'} alt="" fill sizes="80px" className="object-cover" />
      </div>
      <div>
...
export function LabGallery({
  ...
  const spanClass = (span: string) =>
    span === 'tall' ? 'sm:row-span-2 aspect-[3/4]' : span === 'wide' ? 'sm:col-span-2 aspect-[16/9]' : 'aspect-[4/3]'

  return (
    <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {items.map((item, i) => (
        <FadeUp as="li" key={item.src + i} delay={i * 0.04} className={cn('relative', spanClass(item.span))}>
          <figure className="group relative h-full w-full overflow-hidden rounded-2xl">
            ...
            />
            <figcaption className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-charcoal/85 to-transparent p-4 text-sm font-semibold text-white">
              {captions[i] ?? ''}
            </figcaption>
          </figure>
        </FadeUp>
